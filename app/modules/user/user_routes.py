...
def add_new_user_router():
    ...
    if request.method == EMethod.POST:
        ...
        if not validate_phone(request.form['phone']):
            return render_template('user/add_new_user_page.html', error="Неверный номер телефона")
        
        if not validate_password(request.form['password']):
            return render_template('user/add_new_user_page.html', error="Неверный пароль")
        
        if not validate_role(request.form['role']):
            return render_template('user/add_new_user_page.html', error="Неверная роль")
